...
public class DataSelectionWindow extends JDialog {
	...
	public DataSelectionWindow(SalesmanSelectionWindow dialog, Salesman salesman, String fileTypeFlag) {
		inputDialog = dialog;
	...
	public void initialise(){
		getContentPane().setBackground(new Color(238,230,246));
		...
		totalSalesCheckBox.setBackground(new Color(238,230,246));
		...
		totalItemsCheckBox.setBackground(new Color(238,230,246));
		...
		categoryCheckBox.setBackground(new Color(238,230,246));
		...
		shirtRadio.setBackground(new Color(238,230,246));
		...
		trousersRadio.setBackground(new Color(238,230,246));
		...
		coatRadio.setBackground(new Color(238,230,246));
		...
		skirtRadio.setBackground(new Color(238,230,246));
		...
		commissionCheckBox.setBackground(new Color(238,230,246));
		...
		try{
			...
			JOptionPane.showMessageDialog(null,"Salesman's name is not given");
	...
	private void addReceiptButtonPressed(ActionEvent evt) {
	    if (anyFieldNotEmpty()) {
	        if (isReceiptIdAlreadyExists()) {
	            JOptionPane.showMessageDialog(null, "Error: Receipt ID already exists");
	        } else {
	            if (kindTextField.getText().equals("Shirts")) {
	                ...
	                clearTextFields();
	                JOptionPane.showMessageDialog(null, "Error: Invalid clothing type");
	                return;
	            }

	            if (!isValidDateFormat(dateTextField.getText())) {
	                clearTextFields();
	                JOptionPane.showMessageDialog(null, "Error: Invalid date format (MM/dd/yy)");
	                return;
	            }
	...
	private boolean isReceiptIdAlreadyExists() {
	    int newReceiptId;

	    try {
	        newReceiptId = Integer.parseInt(receiptIDTextField.getText());
	    } catch (NumberFormatException e) {
	        JOptionPane.showMessageDialog(null, "Error: Invalid receipt ID");
	        return true; 
	    }


	    for (Receipt existingReceipt : selectedSalesman.getReceipts()) {
	        if (existingReceipt.getReceiptId() == newReceiptId) {
	            return true; 
	        }
	    }
	...
	private void appendFile() {
		AddReceipt fileAppender = selectedSalesman.getFileAppender();

		fileAppender.getReceiptData().setReceiptId(Integer.parseInt(receiptIDTextField.getText()));
		fileAppender.getReceiptData().setDate(dateTextField.getText());
		fileAppender.getKindData().setType(kindTextField.getText());
		fileAppender.getReceiptData().setSales(Double.parseDouble(salesTextField.getText()));
		fileAppender.getReceiptData().setItems(Integer.parseInt(itemsTextField.getText()));

		fileAppender.getCompanyData().setCompany(companyTextField.getText());
		fileAppender.getAddressData().setCountry(countryTextField.getText());
		fileAppender.getAddressData().setCity(cityTextField.getText());
		fileAppender.getAddressData().setStreet(streetTextField.getText());
		fileAppender.getAddressData().setStreetNumber(Integer.parseInt(numberTextField.getText()));

		fileAppender.appendFile();
	}
	...
	private void addReceipt(){
		...
		try{
			...
			receipt.setName(companyTextField.getText());
